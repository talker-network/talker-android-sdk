package network.talker.sdk.player
...
import network.talker.sdk.Talker.eventListener
...
//                        }
                ...
                override fun onPlayerError(error: PlaybackException) {
                    Log.e(LOG_TAG, "Player error: ${error.message}")
                    Log.e(LOG_TAG, "Cause: ${error.cause}")
                    Log.e(LOG_TAG, "Error code: ${error.errorCode}")
                    ...
                    when (error.errorCode) {
                        ...
                        PlaybackException.ERROR_CODE_TIMEOUT -> {
                            Log.e(LOG_TAG, "Network error, skipping to next message")
                            currentLiveMessage = null
                            nextLiveMsg()
                        }
                        // HLS specific errors
                        ...
                        PlaybackException.ERROR_CODE_PARSING_MANIFEST_MALFORMED -> {
                            Log.e(LOG_TAG, "HLS parsing error, skipping to next message")
                            currentLiveMessage = null
                            nextLiveMsg()
                        }
                        // Other errors
                        else -> {
                            Log.d(LOG_TAG, "Attempting simple retry for error: ${error.errorCode}")
                            player?.prepare()
                            player?.play()
                        }
                    }
