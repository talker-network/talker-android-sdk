package com.dev.talkersdk.sdk.socket
...
object SocketHandler {
            ...
            args.forEach { arg ->
                if (arg is String) {
                    runOnUiThread {
                        ...
                        val senderId = JsonParser.parseString(arg).asJsonObject["sender_id"].asString
    ...
    fun closeConnection() {
        ...
        if (socket?.isActive == false || socket?.connected() == false){
            throw Exception("Peer connection is not established. Kindly call create user to connect to the peer")
        }
        socket?.disconnect()
        socket?.off()
        socket?.close()
